...
@router.put("/updatesclprofile/{id}")
def sclprofile(id:int,Post:dict,db : Session = Depends(get_db)):
    data = db.query(schoolProfile.Scl_Profile).filter(schoolProfile.Scl_Profile.id==id)
    if data.first():
        if (Post.get('scholarship')):
            Post['scholarship'] = getData(Post['scholarship'])   
        data.update(Post,synchronize_session=False)
        db.commit()
        return {"Msg":data.first()} 


@router.post("/addsclprofile")
def sclprofile(Post:dict,db : Session = Depends(get_db)):
    new = schoolProfile.Scl_Profile(**Post)
    db.add(new)
    db.commit()
    db.refresh(new)
    return{"data":new.id}
...
@router.get("/retsclprofile")
def retrieve(db : Session = Depends(get_db)):
    retrieve=db.query(schoolProfile.Scl_Profile).all()
    return retrieve

@router.delete("/delprofile/{id}")
def deleteprofile(id:int,db : Session = Depends(get_db)):
    data = db.query(schoolProfile.Scl_Profile).filter(schoolProfile.Scl_Profile.id==id)
    if data.first():
...
@router.get("/specificsclprofile/{id}")
def retrieve(id:int,db : Session = Depends(get_db)):
    retrieve=db.query(schoolProfile.Scl_Profile).filter(schoolProfile.Scl_Profile.id==id).first()
